...
const STRIP = [
  {
    n: "01",
    title: "Куратовано редакцією",
    body: "Кожне оголошення проходить ручний відбір. Жодних шумних «студій за €30» з обкладинками з фотобанку.",
  },
  {
    n: "02",
    title: "Прозорі ціни",
    body: "Усе включено в підсумкову суму до бронювання — без сюрпризів на чек-аут і прихованих сервісних зборів.",
  },
  {
    n: "03",
    title: "Підтримка 24/7",
    body: "Українською, англійською або польською — відповідаємо в чаті протягом години у будь-який час доби.",
  },
  {
    n: "04",
    title: "Гарантія заїзду",
    body: "Якщо щось піде не так на місці — переселимо у житло того ж рівня або повернемо кошти за лічені години.",
  },
];
...
function editorialSlot(i: number) {
  const slot = i % 8;
  if (slot === 0) return { col: "col-span-7", aspect: "aspect-[16/11]" };
  if (slot === 1) return { col: "col-span-5", aspect: "aspect-[4/5]" };
  if (slot === 2) return { col: "col-span-5", aspect: "aspect-[5/4]" };
  if (slot === 6) return { col: "col-span-5", aspect: "aspect-[5/4]" };
  return { col: "col-span-4", aspect: "aspect-[4/3]" };
}
        ...
        <div className="max-w-[1440px] mx-auto px-8 py-16 grid grid-cols-[1.15fr_1fr] gap-14 items-center">
          ...
          <div>
            ...
            <h1 className="font-serif text-[clamp(2.8rem,5.8vw,5.4rem)] leading-[0.97] tracking-[-0.025em] text-[var(--ink)] mb-6">
              Помешкання
              <br />з{" "}
              <em className="italic text-[var(--sh-accent)]">характером</em>
              <br />
              для людей з{" "}
              <span className="relative inline-block">
                ...
                <svg
                  ...
                  aria-hidden
                  className="absolute bottom-[-8px] left-0 w-full h-3"
                >
                  <path
                    d="M2 8C40 2 80 10 120 6S180 4 198 8"
                    fill="none"
            ...
            <p className="text-[16px] leading-[1.55] text-[var(--ink-2)] max-w-[520px] mb-8">
              Понад 580 житл, обраних редакцією: лофти, вілли, кабіни та
              апартаменти у 8 країнах Європи. Без масовки, з характером.
            </p>
          ...
          <div className="relative aspect-[5/6]">
            <div className="absolute top-0 right-0 w-[78%] h-[78%] rounded-[20px] overflow-hidden shadow-[0_16px_48px_rgba(0,0,0,0.12)] z-10 bg-[var(--bg-alt)]">
              ...
              {heroListing && (
                <div className="absolute bottom-0 left-0 right-0 p-5 bg-gradient-to-t from-black/70 to-transparent z-10 text-white">
                  <p className="font-serif text-[22px] leading-[1.1] mb-2">
                    {heroListing.title}
                  </p>
            ...
            <div className="absolute bottom-[-8px] right-[-8px] z-30 flex flex-col bg-[var(--ink)] text-[var(--background)] rounded-2xl px-[18px] py-4 w-[140px] shadow-[0_12px_40px_rgba(0,0,0,0.18)]">
              <span className="font-serif text-[36px] leading-none text-[var(--sh-accent)] mb-1.5">
                580+
              </span>
            ...
            <div className="absolute top-6 left-[-40px] z-30 flex flex-col bg-[var(--surface)] text-[var(--ink)] rounded-2xl px-[18px] py-4 w-[124px] shadow-[0_8px_24px_rgba(0,0,0,0.10)]">
              <span className="font-serif text-[30px] leading-none text-[var(--ink)] mb-1.5">
                4.91
              </span>
            ...
            <div className="flex items-end justify-between gap-8 mb-14">
              <div>
                ...
                <h2 className="font-serif font-normal text-[clamp(2rem,4vw,3.4rem)] leading-none tracking-[-0.02em] text-[var(--ink)]">
                  Помешкання, які
                  <br />
                  <em className="italic text-[var(--sh-accent)]">
                    варто побачити
                  </em>
                </h2>
              </div>
          ...
          <div className="flex items-end justify-between gap-8 mb-14">
            <div>
              ...
              <h2 className="font-serif font-normal text-[clamp(2rem,4vw,3.4rem)] leading-none tracking-[-0.02em] text-[var(--background)]">
                Куди тобі
                <br />
                <em className="italic text-[var(--sh-accent)]">тягне</em> цього
                сезону?
              </h2>
            </div>
            <div className="text-right">
              <span className="block font-serif text-[56px] leading-[0.95] text-[var(--sh-accent)]">
                580+
              </span>
            ...
            {CITIES.map((city, i) => (
              ...
              >
                <div
                  ...
                  style={{
                    background: `linear-gradient(160deg, oklch(0.65 0.12 ${city.hue}) 0%, oklch(0.35 0.10 ${city.hue + 30}) 100%)`,
                  }}
                />
          ...
          <div className="grid grid-cols-4 gap-0">
            {STRIP.map((item, i) => (
              ...
              >
                ...
                <p className="text-[13.5px] text-[var(--ink-2)] leading-[1.55]">
                  {item.body}
                </p>
              </div>
            ))}
        ...
        <div className="max-w-[1320px] mx-auto px-8">
          <div className="relative bg-[var(--ink)] rounded-3xl p-14 overflow-hidden text-[var(--background)]">
            <div className="absolute inset-0 pointer-events-none">
              <div
                className="absolute top-[-250px] right-[-180px] w-[500px] h-[500px] rounded-full opacity-[0.12]"
                style={{
                  background:
                    "radial-gradient(circle, var(--sh-accent) 0%, transparent 70%)",
                }}
              />
              <div
                className="absolute bottom-[-180px] left-[30%] w-[360px] h-[360px] rounded-full opacity-[0.06]"
                style={{
                  background:
                    "radial-gradient(circle, var(--sh-accent) 0%, transparent 70%)",
                }}
              ...
              >
                ...
                />
                <path
                  d="M0 240 Q150 140 300 240 T600 240"
                  stroke="var(--sh-accent)"
                  strokeOpacity="0.12"
                  strokeWidth="1"
                  fill="none"
            ...
            <div className="relative grid grid-cols-[1.3fr_1fr] gap-12 items-center">
              ...
              <div className="flex flex-col">
                {[
                  ...
                  >
                    <span className="font-serif text-[52px] leading-[0.9] text-[var(--sh-accent)]">
                      {val}
                    </span>
